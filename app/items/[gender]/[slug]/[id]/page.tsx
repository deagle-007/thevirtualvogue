...
export default function ProductDetail() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      if (!gender || !slug || !id || (isAdmin && !userEmail)) return;
      ...
      if (isAdmin && userEmail) {
        try {
          ...
          if (res.ok && data.email === userEmail) {
            ...
            console.warn("⚠️ Product doesn't belong to admin or not found.");
            setProduct(null);
          }
        } catch (err) {
        ...
        const staticList = staticProducts[key] || [];
        const match = staticList.find((item) => item.id === id);
    ...
    // ✅ Delay fetch until user is fully loaded
    if (!userLoading) {
      fetchProduct();
    }
  }, [gender, slug, id, isAdmin, userEmail, userLoading]);
  ...
  if (routeLoading || userLoading || loading) return <Loader />;
    ...
    return (
      <div className="min-h-screen flex flex-col justify-center items-center text-center">
        ...
        <p className="text-gray-500">
          Sorry, the requested product does not exist or doesn’t belong to you.
        </p>
